...
def ANIMATE_VIDEO(path, video_path, gen_video_title, packages):
    ...
    def animation(i):
        ...
        # Axis labels
        axes[0].set_xlabel('Distance (x) [$m$]', fontsize=axislabelsize, labelpad=labelpad)
        axes[0].set_ylabel('Distance (y) [$m$]', fontsize=axislabelsize, labelpad=labelpad)
        axes[0].set_zlabel('Distance (z) [$m$]', fontsize=axislabelsize, labelpad=labelpad)
        ...
        for j in range(ns):
            ...
            for i in range(0, (data_set_1.shape[1]-1)//9):
                axes[0].plot(data_set_1[::steps,i*9][imin:imax], data_set_1[::steps,i*9+1][imin:imax], data_set_1[::steps,i*9+2][imin:imax], lw=3, alpha=alpha)

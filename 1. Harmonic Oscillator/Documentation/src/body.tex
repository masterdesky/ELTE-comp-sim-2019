...
Az eredeti forráskódon több változtatást kellett végrehajtani, hogy a kiszabott feladatokat meg lehessen vele oldani. Első lépésben az eredeti \code{sho.cpp} file-ból létrehoztam egy \code{sho\_e.cpp} és egy \code{sho\_ec.cpp} varióciót. Az utóbbi az eredeti Euler-Cromer formulával megadott forráskód volt, míg az első az ezt módosított Euler módszert implementálja. A forráskódokat egy-egy segéd batch file segítségével fordítottam le, abban clang fordítót használva, mely kimeneti binárisa így egy \code{sho\_e.exe} és egy \code{sho\_ec.exe} lett. Az \cite{szamszin} linken elérhető forráskód ellenben egyéb pontokban is apró módosításokra szorult. \\
A második alapvető módosítás eredménye az volt, hogy a szimulációk végleges kimenetként egy \emph{sho.dat} nevű file-t generálnak, ami az adatokat az \ref{tab1}. táblázatnak megfelelő elrendezésben tartalmazza.
...
Az utolsó módosítás a negyedik feladat időméréséhez szükséges módosításokat foglalja magába. Ez összesen egy time() függvény megfelelő modon történő implementlását takarja.
...
\hfill \break \hfill \break
{\centering\includegraphics[width=.5\textwidth]{def_vel_2.eps}}
...
\hfill \break \hfill \break
{\centering\includegraphics[width=.5\textwidth]{def_vel_euler.eps}}
\captionof{figure}{Euler módszer\\Kitérés - Sebesség grafikon}\label{fig4}
\hfill \break \hfill \break
{\centering\includegraphics[width=.5\textwidth]{energy.eps}}
